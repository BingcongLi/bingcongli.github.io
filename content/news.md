# Recent News

- **08/2026.** Tutorial "Low-Rank Adaptation Redux in Large Models" accepted to [MLSP 2026](https://mlsp26.ieeesps.org/event/2026-ieee-international-workshop-on-machine-learning-for-signal-processing/) -- see you in Atlanta!
- **07/2026.** We are orginizing "[On-Device Intelligence](https://odi2026.github.io/)" workshop at NeurIPS 2026 -- see you in Sydney!
- **05/2026.** Check out our tutorial "Low-Rank Adaptation Redux in Large Models" at [ICASSP 2026](https://2026.ieeeicassp.org/tutorials/)!
- **04/2026.** Two papers are accepted to ICML 2026.
- **04/2026.** I will visit Lund University for the Optimization for Learning Symposium.
- **01/2026.** Two papers are accepted to ICLR 2026. We also have an **oral** talk at Sci4DL workshop.
- **01/2026.** [ConMeZO](https://arxiv.org/abs/2511.02757) for memory-efficient LLM fine-tuning is accepted to AISTATS 2026.
- **12/2025.** Talked about "Scaling large efficiently for LLMs" at UC Irvine and University of Minnesota, Twin Cities.
- **10/2025.** A simplified version of [our result](https://arxiv.org/abs/2506.03133) is integrated into the ETH ODS course.
- **10/2025.** Gave a talk at [AI Seminars](https://engineering.oregonstate.edu/EECS/research/AI-seminars) at Oregon State University.
- **10/2025.** Hosted the Efficient LLMs Fine-tuning (ELF) Track at the [AI+X Summit](https://www.plusx.ai/) in Zürich.
- **09/2025.** Talked about the power of weight normalization for fine-tuning LLMs at the [Quantitative Methods and Learning Research Seminar](https://www.unisg.ch/en/university/schools/school-of-economics-and-poltical-science-seps-hsg/research/quantitative-methods-and-learning/), Univ. of St Gallen.
- **09/2025.** Three papers accepted to NeurIPS 2025.
- **07/2025.** Talked about Riemannian optimization and its provable merits for fine-tuning LLMs at EUROPT 2025.
- **06/2025.** Talked about "LoRA Surgery" at the [Efficient Machine Learning Reading Group](https://sites.google.com/view/efficientml).
- **05/2025.** [ICML 2025] Transfer learning provably benefits RLHF. See our [paper](https://arxiv.org/abs/2502.19255v3).


# Old News

- **04/2025.** Talked about "Fine-tuning LLMs cost-efficiently" at Peking University.
- **01/2025.** [ICLR 2025] We prove that [initialization exponentially impacts convergence of ScaledGD](https://arxiv.org/abs/2410.18965) on LoRA-type problems.
- **12/2024.** Talked about "Architecture-Aware Optimization" at ELLIS UnConference.
- **12/2024.** [ICASSP 2025] A new variant of SAM is released.
- **09/2024.** [NeurIPS 2024] We study the [implicit regularization of SAM](https://arxiv.org/abs/2410.14802) and use it to reduce computational burden. Useful for fine-tuning LLMs with LoRA.
- **05/2024.** [ICML 2024] [Memory-efficient private fine-tuning for LLMs](https://arxiv.org/pdf/2310.09639).